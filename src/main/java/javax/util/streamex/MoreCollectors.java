...
public final class MoreCollectors {
    ...
    public static <T, A, D> Collector<T, ?, D> maxAll(Comparator<? super T> comparator, Collector<? super T, A, D> downstream) {
        ...
        Supplier<Object[]> supplier = () -> new Object[3];
        BiConsumer<Object[], T> accumulator = (acc, t) -> {
            if (acc[2] == null) {
                acc[1] = t;
                acc[2] = Boolean.FALSE;
            } else {
                int cmp = comparator.compare(t, (T) acc[1]);
                if (cmp > 0) {
                    acc[0] = null;
                    acc[1] = t;
                    acc[2] = Boolean.FALSE;
                } else if (cmp == 0) {
                    if (acc[2] == Boolean.FALSE) {
                        acc[0] = downstreamSupplier.get();
                        acc[2] = Boolean.TRUE;
                        downstreamAccumulator.accept((A) acc[0], (T) acc[1]);
                    }
                    downstreamAccumulator.accept((A) acc[0], t);
                }
            }
        };
        BinaryOperator<Object[]> combiner = (acc1, acc2) -> {
            if (acc2[2] == null) {
                return acc1;
            }
            if (acc1[2] == null) {
            ...
            int cmp = comparator.compare((T) acc1[1], (T) acc2[1]);
            if (cmp > 0) {
                return acc1;
            }
            if (cmp < 0) {
            ...
            if (acc2[2] == Boolean.FALSE) {
                if (acc1[2] == Boolean.FALSE) {
                    acc1[0] = downstreamSupplier.get();
                    acc1[2] = Boolean.TRUE;
                    downstreamAccumulator.accept((A) acc1[0], (T) acc1[1]);
                }
                downstreamAccumulator.accept((A) acc1[0], (T) acc2[1]);
                return acc1;
            }
            if (acc1[2] == Boolean.FALSE) {
                downstreamAccumulator.accept((A) acc2[0], (T) acc1[1]);
                return acc2;
            }
            acc1[0] = downstreamCombiner.apply((A) acc1[0], (A) acc2[0]);
            return acc1;
        };
        Function<Object[], D> finisher = acc -> {
            if (acc[2] == null)
            ...
            return downstream.finisher().apply((A) acc[0]);
        };
        return Collector.of(supplier, accumulator, combiner, finisher);
    }

    @SuppressWarnings("unchecked")
    public static <T, A, D> Collector<T, ?, D> maxAll2(Comparator<? super T> comparator, Collector<T, A, D> downstream) {
        Supplier<A> downstreamSupplier = downstream.supplier();
        BiConsumer<A, T> downstreamAccumulator = downstream.accumulator();
        BinaryOperator<A> downstreamCombiner = downstream.combiner();
        Supplier<Object[]> supplier = () -> new Object[] {downstreamSupplier.get(), null, Boolean.FALSE};
        BiConsumer<Object[], T> accumulator = (acc, t) -> {
            if(acc[2] == Boolean.FALSE) {
                downstreamAccumulator.accept((A) acc[0], (T) acc[1]);
                acc[1] = t;
        ...
        Function<Object[], D> finisher = acc -> downstream.finisher().apply((A) acc[0]);
        return Collector.of(supplier, accumulator, combiner, finisher);
    }
