...
import one.util.streamex.StreamExInternals.IntCloneableSpliterator;


/**
 * @author Tagir Valeev
 */
/* package */final class IntWithFirstSpliterator extends IntCloneableSpliterator<IntWithFirstSpliterator> implements IntConsumer, Spliterator.OfInt {
    ...
    private IntWithFirstSpliterator prefix;
    ...
    IntWithFirstSpliterator(Spliterator.OfInt source, IntBinaryOperator mapper) {
        this.source = source;
        this.mapper = mapper;
    }
    ...
    public boolean tryAdvance(IntConsumer action) {
        ...
        if (state == STATE_FIRST_READ) {
            state = STATE_INIT;
            action.accept(mapper.applyAsInt(first, first));
            return true;
        }
        if (state != STATE_INIT)
            return false;
    ...
    public Spliterator.OfInt trySplit() {
        ...
        try {
            ...
            if (prefix == null)
                return null;
            IntWithFirstSpliterator result = doClone();
    ...
    public void accept(int x) {
        action.accept(mapper.applyAsInt(first, x));
    }
