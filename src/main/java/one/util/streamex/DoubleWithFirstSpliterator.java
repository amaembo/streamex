...
import one.util.streamex.StreamExInternals.DoubleCloneableSpliterator;


/**
 * @author Tagir Valeev
 */
/* package */final class DoubleWithFirstSpliterator extends DoubleCloneableSpliterator<DoubleWithFirstSpliterator> implements DoubleConsumer, Spliterator.OfDouble {
    ...
    DoubleWithFirstSpliterator(Spliterator.OfDouble source, DoubleBinaryOperator mapper) {
        this.source = source;
        this.mapper = mapper;
    }
    ...
    public boolean tryAdvance(DoubleConsumer action) {
        ...
        if (state == STATE_FIRST_READ) {
            state = STATE_INIT;
            action.accept(mapper.applyAsDouble(first, first));
            return true;
        }
        if (state != STATE_INIT)
            return false;
    ...
    private void doInit() {
        ...
        DoubleConsumer setter = x -> first = x;
    ...
    public Spliterator.OfDouble trySplit() {
        ...
        try {
            ...
            if (prefix == null)
                return null;
            DoubleWithFirstSpliterator result = doClone();
    ...
    public void accept(double x) {
        action.accept(mapper.applyAsDouble(first, x));
    }
